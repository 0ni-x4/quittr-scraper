...
dotenv.config();
const adapter = new FileSync_1.default(path.join(process.cwd(), 'db.json'));
...
async function extractProfileData(page, keyword) {
    try {
        ...
        const category = await page.evaluate(() => document.querySelector('[class*="category"]')?.textContent || undefined);
        ...
        const website = await page.evaluate(() => document.querySelector('header a[href^="http"]')?.getAttribute('href') || undefined);
        return {
            username,
            fullName,
            bio,
...
async function searchAndScrapeProfiles() {
    ...
    const browser = await playwright_1.chromium.launch({ headless: false }); // Set to true in production
